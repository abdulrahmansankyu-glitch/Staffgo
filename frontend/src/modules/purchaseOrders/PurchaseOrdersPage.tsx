...
export function PurchaseOrdersPage() {
  ...
  function load() {
    api.get('/purchase-orders').then((res) => setRows(res.data.purchaseOrders));
  }
  useEffect(() => {
    load();
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setSupplierId(''); setLines([emptyLine()]); setShowForm(false);
    load();
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        {canManage && <Button onClick={() => setShowForm((s) => !s)}>{showForm ? 'Cancel' : '+ New Purchase Order'}</Button>}
      </div>

      {showForm && (
        <Card title="New Purchase Order" className="mb-6">

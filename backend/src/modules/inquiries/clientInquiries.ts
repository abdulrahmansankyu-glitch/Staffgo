import { Router } from 'express';
import { z } from 'zod';
import { prisma } from '../../lib/prisma';
import { requireAuth } from '../../middleware/auth';
import { requireRole } from '../../middleware/rbac';
import { asyncHandler } from '../../middleware/errorHandler';
import { ALL_ROLES, MANAGE_SALES } from '../../lib/roles';
import { nextDocumentNumber } from '../../lib/numbering';

const router = Router();
router.use(requireAuth);

const schema = z.object({
  customerId: z.string(),
  projectId: z.string().optional(),
  subject: z.string().min(1),
  description: z.string().optional(),
  receivedDate: z.coerce.date().optional(),
});

router.get('/', requireRole(...ALL_ROLES), asyncHandler(async (_req, res) => {
  const inquiries = await prisma.clientInquiry.findMany({
    include: { customer: true, quotations: { select: { id: true, number: true } } },
    orderBy: { receivedDate: 'desc' },
  });
  res.json({ inquiries, total: inquiries.length });
}));

router.get('/:id', requireRole(...ALL_ROLES), asyncHandler(async (req, res) => {
  const inquiry = await prisma.clientInquiry.findUnique({
    where: { id: req.params.id },
    include: { customer: true, project: true, quotations: true },
  });
  if (!inquiry) return res.status(404).json({ error: 'Inquiry not found' });
  res.json({ inquiry });
}));

router.post('/', requireRole(...MANAGE_SALES), asyncHandler(async (req, res) => {
  const data = schema.parse(req.body);
  const number = await nextDocumentNumber('CLIENT_INQUIRY');
  const inquiry = await prisma.clientInquiry.create({ data: { ...data, number } });
  res.status(201).json({ inquiry });
}));

router.patch('/:id/status', requireRole(...MANAGE_SALES), asyncHandler(async (req, res) => {
  const { status } = z.object({ status: z.enum(['NEW', 'QUOTED', 'CLOSED']) }).parse(req.body);
  const inquiry = await prisma.clientInquiry.update({ where: { id: req.params.id }, data: { status } });
  res.json({ inquiry });
}));

export default router;
